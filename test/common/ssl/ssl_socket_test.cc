...
void testUtil(const std::string& client_ctx_json, const std::string& server_ctx_json,
  ...
  ServerContextConfigPtr server_ctx_config = std::make_unique<ServerContextConfigImpl>(
      *server_ctx_loader, server.secretManager(), init_manager);
  ...
  ClientContextConfigPtr client_ctx_config = std::make_unique<ClientContextConfigImpl>(
      *client_ctx_loader, server.secretManager(), init_manager);
...
const std::string testUtilV2(const envoy::api::v2::Listener& server_proto,
  ...
  ClientContextConfigPtr client_ctx_config = std::make_unique<ClientContextConfigImpl>(
      client_ctx_proto, server.secretManager(), init_manager);
...
TEST_P(SslSocketTest, FailedClientCertificateSpkiVerificationNoClientCertificate) {
  ...
  testUtilV2(listener, client, "", false, "", "", "", "", "", "ssl.fail_verify_no_cert", 1,
             GetParam());
...
TEST_P(SslSocketTest, FailedClientCertificateSpkiVerificationWrongCA) {
  ...
  testUtilV2(listener, client, "", false, "", "", "", "", "", "ssl.fail_verify_error", 1,
             GetParam());
...
TEST_P(SslSocketTest, FlushCloseDuringHandshake) {
  ...
  Json::ObjectSharedPtr server_ctx_loader = TestEnvironment::jsonLoadFromString(server_ctx_json);
  ServerContextConfigPtr server_ctx_config = std::make_unique<ServerContextConfigImpl>(
      *server_ctx_loader, server_.secretManager(), init_manager_);
  ContextManagerImpl manager(runtime);
  Ssl::ServerSslSocketFactory server_ssl_socket_factory(std::move(server_ctx_config), manager,
                                                        stats_store, std::vector<std::string>{});
...
TEST_P(SslSocketTest, HalfClose) {
  ...
  Json::ObjectSharedPtr client_ctx_loader = TestEnvironment::jsonLoadFromString(client_ctx_json);
  ClientContextConfigPtr client_ctx_config = std::make_unique<ClientContextConfigImpl>(
      *client_ctx_loader, server_.secretManager(), init_manager_);
  ClientSslSocketFactory client_ssl_socket_factory(std::move(client_ctx_config), manager,
                                                   stats_store);
  Network::ClientConnectionPtr client_connection = dispatcher.createClientConnection(
      socket.localAddress(), Network::Address::InstanceConstSharedPtr(),
...
TEST_P(SslSocketTest, ClientAuthMultipleCAs) {
  ...
  Json::ObjectSharedPtr server_ctx_loader = TestEnvironment::jsonLoadFromString(server_ctx_json);
  ServerContextConfigPtr server_ctx_config = std::make_unique<ServerContextConfigImpl>(
      *server_ctx_loader, server.secretManager(), init_manager_);
  ContextManagerImpl manager(runtime);
  Ssl::ServerSslSocketFactory server_ssl_socket_factory(std::move(server_ctx_config), manager,
                                                        stats_store, std::vector<std::string>{});
  ...
  Json::ObjectSharedPtr client_ctx_loader = TestEnvironment::jsonLoadFromString(client_ctx_json);
  ClientContextConfigPtr client_ctx_config = std::make_unique<ClientContextConfigImpl>(
      *client_ctx_loader, server.secretManager(), init_manager_);
...
void testTicketSessionResumption(const std::string& server_ctx_json1,
  ...
  ServerContextConfigPtr server_ctx_config1 = std::make_unique<ServerContextConfigImpl>(
      *server_ctx_loader1, server.secretManager(), init_manager);
  ServerContextConfigPtr server_ctx_config2 = std::make_unique<ServerContextConfigImpl>(
      *server_ctx_loader2, server.secretManager(), init_manager);
...
TEST_P(SslSocketTest, ClientAuthCrossListenerSessionResumption) {
  ...
  ServerContextConfigPtr server2_ctx_config = std::make_unique<ServerContextConfigImpl>(
      *server2_ctx_loader, server_.secretManager(), init_manager_);
  ContextManagerImpl manager(runtime);
  Ssl::ServerSslSocketFactory server_ssl_socket_factory(std::move(server_ctx_config), manager,
                                                        stats_store, std::vector<std::string>{});
  Ssl::ServerSslSocketFactory server2_ssl_socket_factory(std::move(server2_ctx_config), manager,
                                                         stats_store, std::vector<std::string>{});
...
public:
  void initialize() {
    ...
    server_ctx_config = std::make_unique<ServerContextConfigImpl>(
        *server_ctx_loader_, server_.secretManager(), init_manager_);
    ...
    client_ctx_config = std::make_unique<ClientContextConfigImpl>(
        *client_ctx_loader_, server_.secretManager(), init_manager_);
