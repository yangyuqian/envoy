...
void SecretManagerImpl::removeDynamicTlsCertificateProvider(const std::string& map_key) {
  ...
  RELEASE_ASSERT(dynamic_tls_certificate_providers_.erase(map_key) == 1, "");
}
...
TlsCertificateConfigProviderSharedPtr SecretManagerImpl::findOrCreateDynamicSecretProvider(
  ...
  auto secret_provider = dynamic_tls_certificate_providers_[map_key].lock();
  if (!secret_provider) {
...
CertificateValidationContextConfigProviderSharedPtr SecretManagerImpl::findOrCreateDynamicCertificateValidationContextProvider(
  ...
  auto secret_provider = dynamic_certificate_validation_context_providers_[map_key].lock();
  if (!secret_provider) {
    ASSERT(secret_provider_context.initManager() != nullptr);
